#!/usr/bin/env python3
"""Standalone Kimi Code Search skill script."""
...
def _headers(api_key: str, prefix: str, accept: str | None = None) -> dict[str, str]:
    headers = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Kimi Code Search skill")
    ...
    if not api_key:
        raise SystemExit("Kimi Code API Key 未配置")
    ...
    if args.command == "search":
        ...
        text = _post(search_url, body, _headers(api_key, "search"), timeout)
    ...
    text = _post(
        fetch_url,
        {"url": args.url},
        _headers(api_key, "fetch", accept="text/markdown"),
        timeout,
    )
    print(text)
    return 0
