...
def _auth_headers(api_key: str, user_agent: str = "") -> dict[str, str]:
    headers = {
...
def _builtin_web_search(
    *,
    ...
    for _ in range(max_rounds):
        ...
        raw = _post(_chat_url(base_url), body, _auth_headers(api_key, user_agent), timeout)
        data = json.loads(raw)
...
def main() -> int:
    ...
    timeout = int(_cfg(config, "connection_settings", "timeout_seconds", 30) or 30)
    api_mode = str(_cfg(config, "connection_settings", "api_mode", "builtin_web_search") or "builtin_web_search")
    user_agent = str(_cfg(config, "connection_settings", "user_agent", "") or "")

    if args.command == "search":
        if api_mode == "builtin_web_search":
            base_url = os.environ.get("KIMI_CHAT_BASE_URL") or _cfg(
                config, "connection_settings", "chat_base_url", DEFAULT_CHAT_BASE_URL
            )
        ...
        text = _post(
            search_url,
            body,
            _coding_headers(api_key, "search", user_agent=user_agent or KIMI_CODE_USER_AGENT),
            timeout,
        )
    ...
    if api_mode == "builtin_web_search":
        ...
        print(
            _builtin_web_search(
                api_key=api_key,
                query=f"请联网读取这个网页并整理正文要点，保留关键来源：{args.url}",
                base_url=base_url,
                model=model,
                timeout=timeout,
                user_agent=user_agent,
    ...
    text = _post(
        ...
        _coding_headers(
            ...
            user_agent=user_agent or KIMI_CODE_USER_AGENT,
        ),
        timeout,
    )
    print(text)
